...
public enum FlushDraw{
    ...
    public Player calculateWinnerFromHandDraw(Player player1, Player player2) {
        ...
        if (player1Hand.getHandRank() != HandRank.FLUSH) {
            throw new IllegalArgumentException("player 1 Hand must be Flush");
        }
        if (player2Hand.getHandRank() != HandRank.FLUSH) {
            throw new IllegalArgumentException("player 1 Hand must be Flush");
        }
